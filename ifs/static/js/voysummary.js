      ...
      voyages: [
        {
          ...
          route: [
            ...
            },
            {
              ...
              visited: false,
              current: false,
              arrivalDate: "2025-04-26T08:00:00Z",
              purpose: "Discharge",
            },
          ],
        },
...
function createVoyageRow(voyage) {
  ...
  const loadPortTags = createPortTags(voyage.route, "load");
  const destinationTags = createPortTags(voyage.route, "discharge");

  // Get latest trip for fuel and other stats
  const latestTrip = voyage.trips && voyage.trips.length ? voyage.trips[voyage.trips.length - 1] : null;
  const foCons = latestTrip && latestTrip.fuelSummary && latestTrip.fuelSummary["FO"] ? latestTrip.fuelSummary["FO"].sounding_cons : "-";
  const doCons = latestTrip && latestTrip.fuelSummary && latestTrip.fuelSummary["DO"] ? latestTrip.fuelSummary["DO"].sounding_cons : "-";
  const speed = latestTrip && latestTrip.speed ? latestTrip.speed : "-"; // if you have speed field
  const utilization = latestTrip && latestTrip.utilization ? latestTrip.utilization : "-"; // if you have utilization field
  ...
  row.innerHTML = `
        ...
        <td>${latestTrip ? formatDate(latestTrip.end_time) : "-"}</td>
...
function createPortTags(ports, portType) {
  const filteredPorts = ports.filter((port) => port.portType === portType)

  if (filteredPorts.length === 0) {
    return '<span class="svt-no-data">-</span>'
  }

  return filteredPorts.map((port) => `<span class="svt-port-tag">${port.portName}</span>`).join("")
}
